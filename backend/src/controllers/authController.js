...
export const handleGoogleCallback = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    res.redirect(
      `https://mscholar.onrender.com/${state}?success=false&status=AUTH_FAILED&message=Google authentication failed`
    );
  }
  ...
  if (state === 'signup') {
    if (existingUser) {
      res.redirect(
        `https://mscholar.onrender.com/${state}?success=false&status=ACCOUNT_EXISTS&message=Account already exists. Please login instead.`
      );
      return;
    }
    ...
    if (!existingUser) {
      res.redirect(
        `https://mscholar.onrender.com/${state}?success=false&status=ACCOUNT_NOT_FOUND&message=No account found. Please sign up first.`
      );
      return;
    }
