...
export const handleGoogleCallback = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    res.redirect(
      `https://mschloar.onrender.com/${state}?success=false&status=AUTH_FAILED&message=Google authentication failed`
    );
  }
  ...
  if (state === 'signup') {
    if (existingUser) {
      res.redirect(
        `https://mschloar.onrender.com/${state}?success=false&status=ACCOUNT_EXISTS&message=Account already exists. Please login instead.`
      );
      return;
    }
    ...
    res.redirect(`https://mschloar.onrender.com/${state}?success=true&status=AUTH_SUCCESS&message=Google authentication successful&email=${userEmailAddress}&password=${user.id + process.env.JWT_SECRET}`);
    return;
  } else if (state === 'login') {
    if (!existingUser) {
      res.redirect(
        `https://mschloar.onrender.com/${state}?success=false&status=ACCOUNT_NOT_FOUND&message=No account found. Please sign up first.`
      );
      return;
    }
